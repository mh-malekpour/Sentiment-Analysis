...
class MultiHeadedAttention(nn.Module):
    def __init__(self, head_size, num_heads, sequence_length=0):
        ...
        self.W_O = nn.Linear(head_size * num_heads, head_size * num_heads)
        # ==========================
    ...
    def merge_heads(self, tensor):
        ...
        tensor = tensor.transpose(1, 2)
        tensor = tensor.reshape(batch_size, sequence_length, self.num_heads * self.head_size)
    ...
    def forward(self, hidden_states, mask=None):
        ...
        outputs = self.apply_attention(Q, K, V, mask)
...
class PreNormAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask=None):
        # ==========================
        # Apply layer normalization before the attention block
        x = x + self.attn(self.layer_norm_1(x), mask=mask)
